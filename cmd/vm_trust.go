...
func (c *VmTrustCommand) Run(args []string) int {
	...
	// The WSL backend has no pkg/trust store; import certs from the distro into
	// the Windows Trusted Root store via certutil instead.
	if runtime.GOOS == "windows" {
		wslManager, ok := manager.(*wsl.Manager)
		if !ok {
			c.UI.Error("'trellis vm trust' requires the WSL backend on Windows.")
			return 1
		}
